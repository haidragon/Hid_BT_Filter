...
)
{
	...
	size_t InputHeaderSize = sizeof(ptpOutputReport);
	...
	if (responseLength < InputHeaderSize) {
    ...
    ptpOutputReport = *(PTP_REPORT*)TouchDataBuffer;
    KdPrint(("PtpFilterInputRequestCompletionCallback ptpOutputReport.Contacts[0].X =,%x\n", ptpOutputReport.Contacts[0].X));

	// Report header
	ptpOutputReport.ReportID = FAKE_REPORTID_MULTITOUCH;
	ptpOutputReport.IsButtonClicked = 0;

	ptpOutputReport.ContactCount = 1;


	ptpOutputReport.Contacts[0].ContactID = 0;
	ptpOutputReport.Contacts[0].X = 1;
	ptpOutputReport.Contacts[0].Y = 1;
	ptpOutputReport.Contacts[0].TipSwitch = 1;

	ptpOutputReport.Contacts[0].Confidence = 1;
